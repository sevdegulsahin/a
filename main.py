...
import base64 # grafiği html'in algılaması için base64 formatına dönüştürdüm
import io
...
# stacking model regression, one hot encoding ve standardscaler işlemlerini kullanmak için yükleme
model = joblib.load(r"C:\Projects\hackathon_deneme\a\stacking_model.pkl")
ohe = joblib.load(r"C:\Projects\hackathon_deneme\a\ohe.pkl")
scalers = joblib.load(r"C:\Projects\hackathon_deneme\a\standard_scalers.pkl")
...
def preprocess_user_input(user_input):
    # categorical sütunları işleme
    categorical_data = pd.DataFrame({col: [user_input.get(col, 'None')] for col in cat_cols})

    # Recycling ve Cooking_With sütunlarının liste formatını string'e çevirme
    ...
    # one hot encoding ile categoricalları kodlama
    ...
    # numerical sütunları işleme
    numerical_data = pd.DataFrame({col: [float(user_input.get(col, 0))] for col in num_cols})

    # numerical sütunları standardscaler kullanarak scale etme
    for col in num_cols:
        if col in scalers:
            numerical_data[col] = scalers[col].transform(numerical_data[[col]])

    # numerical ve categorical verileri birleştirme (concat)
    processed_data = pd.concat([numerical_data, categorical_encoded_df], axis=1)

    # XGBoost Regression'un çalışması için gereken tüm featuresları ekleme (eksik olanlar için 0 verdim yoksa en ufak şeyde one hot encoding sapıtıp hata veriyor)
    for col in model.feature_names_in_:
        if col not in processed_data.columns:
            processed_data[col] = 0

    # XGBoost Regression beklediği sütunları input sırasına göre düzenleme (yoksa yine hata veriyor :))
...
def carbon_footprint_graph(prediction):
    ...
    plt.axis('equal') # pasta grafiği dairesel olsun

    # html için base64 formatına çevirme
    buffer = io.BytesIO()
...
async def predict(
    ...
    try:
        ...
        # XGBoost Regression ile karbon ayak izi tahmini
        ...
        return templates.TemplateResponse(
            "index.html",
            {"request": request, "prediction": f"Estimated Carbon Emission: {prediction:.2f} kg CO2/year", "plot_data": plot_data}
        )
